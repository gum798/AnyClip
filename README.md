...
- **버전 협상**: handshake에서 양쪽이 protocol_major/minor를 교환해 메이저 불일치 시 link 거부 + 메뉴바 경고. 현재 프로토콜은 1.2이며 minor는 누적 기능 레벨이다(≥1이면 `kind:"files"`, ≥2면 64MiB 프레임 수신 가능).
...
- **여러 파일 동기화**: 파일을 2개 이상 복사하면 하나의 `kind:"files"` 프레임(프로토콜 1.1)으로 묶어 전송. 합계 예산(~49MB)을 넘거나 100개를 초과하는 파일은 건너뛰고 알림으로 표시. 상대가 프로토콜 1.0(구버전)이면 첫 파일만 전송. 폴더는 동기화하지 않음.
